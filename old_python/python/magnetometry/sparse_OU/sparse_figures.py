...
plt.figure()
plt.plot(t_autocorrelation, true_state_autocorrelation, label='B-field autocorrelation')
plt.plot(t_autocorrelation, P_max_autocorrelation, label='B_{PQS} autocorrelation')
plt.xlabel('Displacement')
plt.ylabel('Autocorrelation amplitude')
plt.title('B-field and B_{pqs} autocorrelation functions')
plt.legend(loc="upper right")
plt.savefig(image_folder + 'autocorrelation' + image_type)
...
weighted_width_average_of_t = (P * delta_s[:, np.newaxis]).mean(axis=0)
...
weighted_width_average_of_t_forward = (P_f * delta_s[:, np.newaxis]).mean(axis=0)
