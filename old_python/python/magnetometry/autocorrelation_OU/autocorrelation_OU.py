...
if __name__ == '__main__':    
    ...
    gamma_dec = 1 * np.ones(N) #np.linspace(0, 2, N)
    gamma_phi = 1 * np.ones(N) #np.linspace(0, 2, N)
    ...
    with open('autocorrelation_N_{0}_T_{1}_r_{2}_'.format(N, T, base_rate) + datetime.datetime.now().strftime("%H_%M_%S_%d_%m_%y") + ".pkl", 'wb') as f:
        pickle.dump([t_autocorrelation, true_state_autocorrelation, P_max_autocorrelation], f)
